...
class PatchGCN_Surv(torch.nn.Module):
    def __init__(self, input_dim=1536, num_layers=4, edge_agg='spatial', multires=False, resample=0,
                 fusion=None, num_features=768, hidden_dim=128, linear_dim=64, use_edges=False, pool=False,
    ...
    def forward(self, x_img_256,x_img_256_edge): #**kwargs
        # data = x#kwargs['x_path']
        x_img_cnn_edge = x_img_256_edge
        x_img_cnn = x_img_256
        if self.edge_agg == 'spatial':
            edge_index = x_img_cnn_edge#edge_index
        # elif self.edge_agg == 'latent':
        #     edge_index = data.edge_latent
        ...
        logits = self.classifier(h).unsqueeze(0)  # logits needs to be a [1 x 4] vector
        Y_hat= torch.topk(logits, 1, dim=1)[1]
        Y_prob = F.softmax(logits, dim=1)#torch.sigmoid(logits)
        hazards = torch.sigmoid(logits)
        S = torch.cumprod(1 - hazards, dim=1)

        return logits, Y_prob, Y_hat,A_path
        # return hazards, S, Y_hat, A_path#, None