...
import numpy as np
from Models.network import Classifier_1fc, DimReduction
...
class IBMIL(nn.Module):
    def __init__(self, conf, confounder_dim=128, confounder_merge='cat'):
        ...
        self.classifier = Classifier_1fc(conf.D_inner, conf.num_classes, 0)
        ...
        if conf.c_path:
            ...
            if confounder_merge == 'cat':
                self.classifier = nn.Linear(conf.D_inner + conf_tensor_dim, conf.num_classes)
            elif confounder_merge == 'add' or 'sub':
                self.classifier = nn.Linear(conf.D_inner, conf.num_classes)
