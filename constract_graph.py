...
folder_path_256 = 'M:\\STAS_multis\\data\\xaingya2'#'R:\\zhengda\\np'
# folder_path_512 = 'U:\\lung_TP53_2025\\features\\6-16\\512\\FEATURES_DIRECTORY\\pt_files'#'R:\\zhengda\\np_swim'
# folder_path_1024 = 'U:\\lung_TP53_2025\\features\\6-16\\1024\\FEATURES_DIRECTORY\\pt_files'
# save_path_swim = 'P:\\xiangya2\\STAS\\stas_all\\graph\\swim'
# save_path_all = 'P:\\xiangya2\\STAS\\stas_all\\graph\\cnn_swim'
path = "M:\\STAS_multis\\data\\xiangya2_graph"
all_data_cnn = {}
all_data_swim = {}
all_data = {}
# 读取每个文件夹内的所有文件名（不含路径）

# 获取每个文件夹下的 .pt 文件名（去掉扩展名）
files_256 = {os.path.splitext(f)[0] for f in os.listdir(folder_path_256) if f.endswith('.pt')}
# files_512 = {os.path.splitext(f)[0] for f in os.listdir(folder_path_512) if f.endswith('.pt')}
# files_1024 = {os.path.splitext(f)[0] for f in os.listdir(folder_path_1024) if f.endswith('.pt')}

# 取三者交集
# first_column_data = list(files_256 & files_512 & files_1024)
...
for file_name in files_256:
    ...
    # file_name = "1590202-7-HE"
    image_path_256_fea = os.path.join(folder_path_256, file_name+'.pt')
    # feats_csv_path1 = image_path_256_fea.replace('256\\FEATURES_DIRECTORY\\pt_files\\', 'muti_graph\\')
    # h5_path = feats_csv_path1.replace('.pt', '.h5')
    # from pathlib import Path
    #
    # h5_path = Path(h5_path)  # 如果它还是个字符串的话
    # # 如果目标已存在就跳过
    # if h5_path.exists():  # 检查文件是否存在 :contentReference[oaicite:4]{index=4}
    #     print(f"已存在，跳过：{h5_path}")
    #     continue
    image_path_256_fea = torch.load(image_path_256_fea)

    # image_path_512_fea = os.path.join(folder_path_512, file_name+'.pt')
    # image_path_512_fea = torch.load(image_path_512_fea)
    #
    # image_path_1024_fea = os.path.join(folder_path_1024, file_name+'.pt')
    ...
    h5_file_path = 'M:\\STAS_multis\\data\\xiangya2_graph\\{}.h5'.format(file_name.split('.')[0])
